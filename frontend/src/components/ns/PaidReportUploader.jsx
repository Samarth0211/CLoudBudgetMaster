...
/**
 * Post-payment CSV uploader. Shown after /bill-audit/capture succeeds.
 * POSTs to /bill-audit/paid-report multipart { token, file } ->
 * { ok, report } and renders the full findings inline. Report is also
 * emailed server-side, so we show the confirmation email address.
 */
export default function PaidReportUploader({ token, email }) {
  ...
  const onFileChange = async (e) => {
    ...
    try {
      const res = await api.post('/bill-audit/paid-report', formData, {
      ...
      setError(typeof detail === 'string' ? detail : 'Could not process that file. Try exporting a fresh billing CSV and upload again.')
      setStatus('error')
    } finally {
    ...
    <div className="flex flex-col gap-5">
      ...
      {status === 'idle' && (
        ...
        >
          ...
          <p className="mt-1.5 text-[12px]" style={{ color: 'var(--cbm-fg-4)' }}>AWS Cost and Usage Report or billing export. File deleted after processing.</p>
        </div>
      )}
      ...
      {status === 'done' && report && (
        <div className="flex flex-col gap-4">
          {email && (
            <p className="text-[12.5px]" style={{ color: 'var(--cbm-fg-3)' }}>
              We also emailed this report to <span className="font-mono">{email}</span>.
            </p>
          )}
