...
function sectionTitle(ctx, text, note) {
  ensure(ctx, 40)
  const { doc } = ctx
  doc.setTextColor(...INK); doc.setFont('helvetica', 'bold'); doc.setFontSize(13)
  doc.text(text, MARGIN, ctx.y)
  if (note) { doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(...GREY); doc.text(note, MARGIN + doc.getTextWidth(text) + 8, ctx.y) }
...
function rankedBars(ctx, items, color) {
  ...
  items.forEach((it) => {
    ...
    doc.text(it.label, MARGIN, ctx.y + 8, { maxWidth: labelW - 6 })
...
export async function generateCostReport({ summary, trend = [], connections = [], resources = [], services = [], forecast, tags, account }) {
  ...
  const cards = [
    ['Monthly Spend', money(monthly), INK, wow !== 0 ? `${wow > 0 ? '▲' : '▼'} ${Math.abs(wow)}% WoW` : ''],
  ...
  // Cost by service
  if (services.length) {
    sectionTitle(ctx, 'Cost by Service', '(last 30 days)')
    rankedBars(ctx, services.slice(0, 10).map(s => ({ label: svcLabel(s.service), value: s.cost, percent: s.percent })), ORANGE)
  }

  // Cost trend
  if (trend.length) { sectionTitle(ctx, '30-Day Cost Trend'); drawTrendChart(ctx, trend) }
  ...
  if (groups.length) {
    ...
    sectionTitle(ctx, 'Cost Allocation', `by ${tags.tag_key} · tagging coverage ${taggedPct}%`)
  ...
  if (topCost.length) {
    sectionTitle(ctx, 'Top Resources by Cost')
    autoTable(doc, {
      startY: ctx.y,
  ...
  if (wastedRes.length) {
    ensure(ctx, 60); sectionTitle(ctx, 'Recommended Remediation')
